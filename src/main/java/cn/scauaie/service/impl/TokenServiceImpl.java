...
public class TokenServiceImpl implements TokenService {
    ...
    public Result<TokenAO> saveToken(TokenAO tokenAO, int seconds) {
        ...
        if (!code.equals(RedisStatus.OK.name())) {
            ...
            return Result.fail(ErrorCode.INTERNAL_ERROR.getError(), "Failed to create token.");
        }
        ...
        if (result.equals(0L)) {
            ...
            return Result.fail(ErrorCode.INTERNAL_ERROR.getError(), "Failed to set expire.");
        }
     ...
     */
    @Override
    public Result<TokenAO> getToken(HttpServletRequest request) {
        ...
        //token不在
        if (jsonToken == null) {
            return Result.fail(ErrorCode.UNAUTHORIZED.getError(), ErrorCode.UNAUTHORIZED.getMessage());
        }
...
//     */
//    @Override
//    public Result<TokenAO> authTokenAndSetExpire(HttpServletRequest request) {
//        Result<TokenAO> result = getToken(request);
//        if (!result.isSuccess()) {
//            return result;
//        }
//
//        cacheService.expire(result.getData().getToken(), TokenExpire.NORMAL.getExpire());
//        return result;
//    }
     ...
     */
    @Override
    public Result<TokenAO> authTokenAndSetExpire(HttpServletRequest request, TokenType tokenType) {
        Result<TokenAO> result = getToken(request);
        if (!result.isSuccess()) {
            return result;
        }
        ...
        if (!result.getData().getType().equals(tokenType.name())) {
            return Result.fail(ErrorCode.FORBIDDEN_SUB_USER.getError(), ErrorCode.FORBIDDEN_SUB_USER.getMessage());
        }
